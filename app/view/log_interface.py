...
from qfluentwidgets import (qrouter, ScrollArea, PushButton, ToolButton, FluentIcon,
                            isDarkTheme, IconWidget, Theme, ToolTipFilter, TitleLabel, CaptionLabel,
                            StrongBodyLabel, BodyLabel, toggleTheme, TextEdit, ListWidget, Pivot)
...
class GalleryInterface(ScrollArea):
    ...
    def __init__(self, title: str, subtitle: str, parent=None):
        ...
        self.terminal = PivotInterface(self.view)
...
class PivotInterface(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        # add items to pivot
        initInterface = self.addSubInterface('Terminal1', '证明日志')
        self.addSubInterface('Terminal2', '验证日志')
        self.addSubInterface('Terminal3', '系统日志')
        ...
        self.stackedWidget.setCurrentWidget(initInterface)
        self.pivot.setCurrentItem(initInterface.objectName())

        qrouter.setDefaultRouteKey(self.stackedWidget, initInterface.objectName())

    def addSubInterface(self, objectName, text):
        card = TerminalCard(self)
        card.setObjectName(objectName)
        self.stackedWidget.addWidget(card)
        self.pivot.addItem(
            routeKey=objectName,
            text=text,
            onClick=lambda: self.stackedWidget.setCurrentWidget(card)
        )
        return card
...
class TerminalCard(QWidget):
    '''used for create a terminal in pivot'''
    ...
    def __init__(self, parent=None):
        ...
        self.view = QFrame()
        self.hlayout = QHBoxLayout(self.view)
        self.terminal = TextEdit(self.view)
        self.list = ListFrame(self.view)

        self._initLayout()
        signalBus.proof.connect(self._appendText)
    
    def _appendText(self, line):
        self.terminal.append(line.rstrip())

    def _initLayout(self):
        # self.vBoxLayout.addWidget(self.terminal, 1)
        ...
        self.hlayout.addWidget(self.terminal, 1)

        # self.terminal.setMarkdown("## Hello \n this is terminal")
        # self.terminal.setFixedHeight(500)

        # self.list.setFixedSize(500, 300)
...
class ListFrame(QFrame):
    ...
    def __init__(self, parent=None):
        ...
        # temporily use "Day" unit, later change to minutes
        stands = [
           '2023年10月4日', '2023年10月5日', '2023年10月6日', 
           '2023年10月7日', '2023年10月8日', '2023年10月9日', 
           '2023年10月10日', '2023年10月11日', '2023年10月12日', 
        #    '2023年10月4日', '2023年10月5日', '2023年10月6日', 
        #    '2023年10月7日', '2023年10月8日', '2023年10月9日', 
        #    '2023年10月10日', '2023年10月11日', '2023年10月12日', 
        #    '2023年10月4日', '2023年10月5日', '2023年10月6日', 
        ...
        # only create the first log, 
        # whenever pushing the botton, create a new log
        for stand in stands:
